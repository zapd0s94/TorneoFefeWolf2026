...
def borrar_historial_detalle_gp(gp: str):
    con = _hist_det_db()
    try:
        con.execute("DELETE FROM historial_detalle WHERE gp = ?", (str(gp).strip(),))
        con.commit()
    finally:
        con.close()
...
def conectar_google_sheets(nombre_hoja="sheet1"):
    ...
    try:
        ...
        st.sidebar.success("✅ Conectado a Google Sheets OK")
        ...
        st.sidebar.error(f"ERROR Google Sheets: {repr(e)}")
        return None
