...
def flecha_arriba_derecha():
    components.html(
        ...
        (function () {
          ...
          const oldBtn = PARENT_DOC.getElementById("fw-top-btn");
          if (oldBtn) oldBtn.remove();

          const btn = PARENT_DOC.createElement("button");
          btn.id = "fw-top-btn";
          btn.innerHTML = "⬆";
          btn.title = "Ir arriba";
          ...
          btn.style.boxShadow = "0 10px 30px rgba(0,0,0,.55)";
          ...
          btn.onclick = function () {
            ...
            try {
              const app = PARENT_DOC.querySelector('[data-testid="stAppViewContainer"]');
              if (app) app.scrollTop = 0;
            } catch(e) {}

            try {
              const sectionMain = PARENT_DOC.querySelector('section.main');
              if (sectionMain) sectionMain.scrollTop = 0;
            } catch(e) {}

            try {
              const block = PARENT_DOC.querySelector('.block-container');
              if (block) block.scrollIntoView({behavior: "smooth", block: "start"});
            } catch(e) {}
          };

          PARENT_DOC.body.appendChild(btn);
        })();
        </script>
        """,
        height=0,
...
def persist_token_later(t: str):
    st.session_state["_fw_token_pending"] = t
...
def ls_get(key: str) -> str:
    components.html(
        ...
        <script>
          ...
          url.searchParams.set("_ls", v);
          window.history.replaceState(null, "", url.toString());
        </script>
        """,
        height=0,
    )
    return st.query_params.get("_ls", "")

def floating_scroll_top_button():
    """Botón flotante a la DERECHA que hace scroll real en Streamlit (desktop + mobile)."""
    components.html(
        """
        <style>
          #fwTopBtn{
          ...
          #fwTopBtn:active{ transform: scale(.95); }
        </style>

        <div id="fwTopBtn" title="Ir arriba">⬆️</div>

        <script>
          (function(){
            const btn = document.getElementById('fwTopBtn');
            if(!btn) return;
            ...
            function goTop(){
              ...
              try{
                ...
                if(view) view.scrollTo({top:0, left:0, behavior:'smooth'});
...
def render_dark_table(df: pd.DataFrame):
    """Tabla oscura + Posición con medallas (si tiene Piloto y Puntos)."""
    dfx = df.copy()
    cols = [c.lower() for c in dfx.columns]
    if ("puntos" in cols) and ("piloto" in cols) and ("pos" not in cols) and ("posición" not in cols):
        medals = {1: "🥇", 2: "🥈", 3: "🥉"}
        pos = []
        for i in range(len(dfx)):
            p = i + 1
            pos.append(f"{medals.get(p, '🏁')} {p}")
        dfx.insert(0, "Pos", pos)

    # render HTML controlado para evitar tabla blanca de st.dataframe
    html = dfx.to_html(index=False, escape=False)
...
def is_logged_in():
    # logueado si hay perfil o si hay usuario (por si el token/refresh tarda)
    perfil_ok = st.session_state.get("perfil") is not None
    usuario_ok = bool(st.session_state.get("usuario"))
    return perfil_ok or usuario_ok
...
def logout():
    ...
    # 4) borra token del navegador (localStorage)
    try:
        components.html(
            """
            <script>
              try{ localStorage.removeItem('fw_token'); }catch(e){}
            </script>
            """,
            height=0,
        )
    ...
    if (not is_logged_in()) and token:
        ...
        if u:
            ...
            if perfil_auto:
                ...
                st.rerun()
    # Si no hay token en la URL, intentá recuperarlo desde localStorage y recargar con ?t=
    # (esto es lo más estable para que F5 no desloguee)
    if not qp_get_one("t"):
        components.html(
            """
            <script>
...
def main():
    ...
    # Flecha flotante (derecha) para subir arriba (desktop + mobile)
    floating_scroll_top_button()
        
        
if __name__ == "__main__":
        main()   
flecha_arriba_derecha()